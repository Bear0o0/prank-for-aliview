...
//        cout<<tree<<endl<<rooted<<endl;
    ...
    else
    {
        ...
        // Rename best alignment to the user-specified output filename (-o)
        //
        {
            string bestFile = filename + this->formatExtension(format);
            string finalFile = outfile;
            if (bestFile != finalFile)
            {
                if (rename(bestFile.c_str(), finalFile.c_str()) == 0)
                    cout<<" - final alignment to '"<<finalFile<<"'\n";
                else
                    cout<<" - warning: could not rename '"<<bestFile<<"' to '"<<finalFile<<"'\n";
            }
        }
